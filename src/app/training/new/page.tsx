...
export default function NewTrainingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.from('training_session').insert([sessionData]).select();
  ...
  const breadcrumbs = [
    ...
    { label: 'New Training Log', isActive: true },
  ];
  ...
  return (
    <DashboardLayout breadcrumbs={breadcrumbs}>
      <Card className="w-full  mx-auto border-none">
        <CardHeader>
          <CardTitle className="text-3xl font-bold">Log Training Session</CardTitle>
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6" aria-label="Training session form">
            ...
            {success && <div className="text-green-600 text-center">Training session logged successfully!</div>}
          </form>
        </CardContent>
      </Card>
    </DashboardLayout>
