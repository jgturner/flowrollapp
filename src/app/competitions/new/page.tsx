...
export default function NewCompetitionPage() {
  ...
  const [competitionDate, setCompetitionDate] = useState<Date>(today);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let podiumPhotoUrl = null;
      if (podiumPhoto) {
        podiumPhotoUrl = await uploadPhotoToStorage(podiumPhoto);
        if (!podiumPhotoUrl) {
      ...
      const { error } = await supabase.from('competitions').insert([competitionData]).select();
  ...
  const breadcrumbs = [
    ...
    { label: 'Add Competition', isActive: true },
  ];
  ...
  return (
    <DashboardLayout breadcrumbs={breadcrumbs}>
      <Card className="w-full mx-auto border-none">
        <CardHeader>
          <CardTitle className="text-3xl font-bold">Add Competition</CardTitle>
          <CardDescription>Record your competition details and results</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6" aria-label="Competition form">
              ...
              <div className="space-y-4">
                {podiumPhotoPreview ? (
                  <div className="relative w-full max-w-md">
                    <Image src={podiumPhotoPreview} alt="Podium photo preview" width={400} height={300} className="rounded-lg object-cover" />
            ...
            <div className="flex justify-end gap-4">
              ...
              <Button type="submit" disabled={loading || uploadingPhoto} className="min-w-32">
                {loading ? 'Adding...' : success ? 'Added!' : 'Add Competition'}
              </Button>
            </div>
            ...
            {/* Success Message */}
            {success && <div className="text-green-600 text-sm text-center bg-green-50 p-3 rounded">Competition added successfully! Redirecting...</div>}
          </form>
        </CardContent>
      </Card>
    </DashboardLayout>
